...
const ManageOut = () => {
    ...
    const handleFileChange = async (event) => {
        ...
        try {
            let res = await authApi().post(endpoints['recognize-out'], form, {
                headers: {
            ...
            if (res.status == 201) {
                ...
                if (res.data.inImgUrl != null)
                    setInImgUrl(res.data.inImgUrl);
                else
                    setInImgUrl("https://via.placeholder.com/150");
            ...
            if (res.data == "No entered vehicle found!"){
                setLoading(false);
                setMsg("Không tìm thấy dữ liệu vào của xe");
                setData(null);
    ...
    const uploadEntryInfo = async (e) => {
        ...
        {
            form.append('id', entryChoice.id);
            form.append('timeOut', data.timeOut);
        }
        else if (files){
            form.append('file', files.current.files[0]);
            form.append('licensePlateNum', data.licensePlateNum);
            form.append('timeOut', data.timeOut);
        }

        try {
            let res = await authApi().post(endpoints['recognize-out-save'], form, {
                headers: {
                    'Content-Type': 'multipart/form-data',
                },
            });
    ...
    const loadEntries = async () => {
        try {
            let url = `${endpoints['entry-in-list']}?plateLicense=${plateLicense}`;
    ...
    const chooseEntry = (e, entry) => {
        e.preventDefault();
        setEntryChoice(entry);
        closeVehicle();
    };
    ...
    return <>
        <div className="container">
            ...
            <div className='divBtn'>
                <Button className='btn btn-info btnFind' onClick={showVehicle}>Tra cứu lịch sử</Button>
            </div>
            ...
            <div className="form-container">
                ...
                <div className="time-container">
                    ...
                    <div className="form-group">
                        <input disabled id="checkOutTime" type="text" placeholder="Thời gian ra" value={data? formatDateWithHour(data.timeOut) : "Thời gian ra"} />
                    </div>
                </div>
            ...
            <Modal.Body className='modal-body'>
                <div className="modal-container">
                    ...
                    <div className="tableEntry">
                        ...
                        <tbody>
                            {entries.length > 0 && entries.map(entry => <tr key={entry.id}>
                                <td>{entry.id}</td>
                                <td>{entry.vehicle.name}</td>
                                <td>{entry.vehicle.plateLicense}</td>
                                <td>{formatDateWithHour(entry.timeIn)}</td>
                                <td>
                                    <Button onClick={(e) => chooseEntry(e, entry)}>Chọn</Button>
                                </td>
                            </tr>)}
                        </tbody>
                    </div>
